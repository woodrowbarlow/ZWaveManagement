...
#include <jni.h>
#include "SerialNativeInterface.h"
...
JNIEXPORT jlong JNICALL Java_jssc_SerialNativeInterface_openPort
  (JNIEnv *env, jobject object, jstring portName, jboolean useTIOCEXCL) {
#ifdef DEBUG
    fprintf(stderr, "%s\n", __FUNCTION__);
#endif
  ...
  (JNIEnv *env, jobject object, jlong portHandle, jint baudRate, jint byteSize, jint stopBits, jint parity, jboolean setRTS, jboolean setDTR, jint flags) {
    ...
    if(tcgetattr(portHandle, settings) == 0){
        if(baudRateValue != -1){
            ...
            struct serial_struct *serial_info = malloc(sizeof *serial_info);
...
#ifdef TIOCGICOUNT
    struct serial_icounter_struct *icount = malloc(sizeof *icount);
...
JNIEXPORT jobjectArray JNICALL Java_jssc_SerialNativeInterface_waitEvents
  (JNIEnv *env, jobject object, jlong portHandle) {
    ...
    jobjectArray returnArray = (*env)->NewObjectArray(env, 11, intClass, NULL);
    ...
    for(i = 0; i < 11; i++){
        ...
        switch(i) {
            ...
            case 0: //Interrupt Break - for BREAK event
		returnValues[0] = INTERRUPT_BREAK;
                returnValues[1] = interruptBreak;
                goto forEnd;
            case 1: //Interrupt TX - for TXEMPTY event
		returnValues[0] = INTERRUPT_TX;
                returnValues[1] = interruptTX;
                goto forEnd;
            case 2: //Interrupt Frame - for ERR event
		returnValues[0] = INTERRUPT_FRAME;
                returnValues[1] = interruptFrame;
                goto forEnd;
            case 3: //Interrupt Overrun - for ERR event
		returnValues[0] = INTERRUPT_OVERRUN;
                returnValues[1] = interruptOverrun;
                goto forEnd;
            case 4: //Interrupt Parity - for ERR event
		returnValues[0] = INTERRUPT_PARITY;
                returnValues[1] = interruptParity;
                goto forEnd;
            case 5:
		returnValues[0] = EV_CTS;
                returnValues[1] = statusCTS;
                goto forEnd;
            case 6:
		returnValues[0] = EV_DSR;
                returnValues[1] = statusDSR;
                goto forEnd;
            case 7:
		returnValues[0] = EV_RING;
                returnValues[1] = statusRING;
                goto forEnd;
            case 8: /*DCD*/
		returnValues[0] = EV_RLSD;
                returnValues[1] = statusRLSD;
                goto forEnd;
            case 9:
		returnValues[0] = EV_RXCHAR;
                returnValues[1] = bytesCountIn;
                goto forEnd;
            case 10:
		returnValues[0] = EV_TXEMPTY;
                returnValues[1] = bytesCountOut;
                goto forEnd;
        }
        forEnd: {
            jintArray singleResultArray = (*env)->NewIntArray(env, 2);
            (*env)->SetIntArrayRegion(env, singleResultArray, 0, 2, returnValues);
            (*env)->SetObjectArrayElement(env, returnArray, i, singleResultArray);
        };
    }
    return returnArray;
}
...
JNIEXPORT jbyteArray JNICALL Java_jssc_SerialNativeInterface_readBytes
  (JNIEnv *env, jobject object, jlong portHandle, jint byteCount) {
...
#endif
    fd_set read_fd_set;
    jbyte *lpBuffer = malloc(byteCount*sizeof(jbyte));
    ...
    jbyteArray returnArray = (*env)->NewByteArray(env, byteCount);
    (*env)->SetByteArrayRegion(env, returnArray, 0, byteCount, lpBuffer);
    free(lpBuffer);
    return returnArray;
}
...
JNIEXPORT jboolean JNICALL Java_jssc_SerialNativeInterface_writeBytes
  (JNIEnv *env, jobject object, jlong portHandle, jbyteArray buffer) {
    ...
    fprintf(stderr, "%s\n", __FUNCTION__);
#endif
    jbyte* jBuffer = (*env)->GetByteArrayElements(env, buffer, JNI_FALSE);
...
JNIEXPORT jintArray JNICALL Java_jssc_SerialNativeInterface_getBuffersBytesCount
  (JNIEnv *env, jobject object, jlong portHandle) {
    ...
    (*env)->SetIntArrayRegion(env, returnArray, 0, 2, returnValues);
    return returnArray;
}
...
JNIEXPORT jintArray JNICALL Java_jssc_SerialNativeInterface_getLinesStatus
  (JNIEnv *env, jobject object, jlong portHandle) {
    ...
    jintArray returnArray = (*env)->NewIntArray(env, 4);
    ...
    (*env)->SetIntArrayRegion(env, returnArray, 0, 4, returnValues);
    return returnArray;
}
  ...
  (JNIEnv *env, jobject object, jlong portHandle, jint duration) {
    ...
    if(duration > 0){
        if(ioctl(portHandle, TIOCSBRK, 0) >= 0){
            ...
            struct timespec *timeStruct = malloc(sizeof *timeStruct);
