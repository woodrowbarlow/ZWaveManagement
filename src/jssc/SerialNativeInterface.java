...
public class SerialNativeInterface {
    ...
    static {
        loadLib("jSSC-2.8_mips");
    }
    
    /**
     * Attempt to load the native library.
     * 
     * @param libFilePath Absolute path to library file.
     * @return true if the library was loaded, false otherwise.
     */
    private static boolean loadLib(String libName) {
    	logger.debug("Attempting to load library.");
        try {
        	System.loadLibrary("jSSC-2.8_mips");
        	logger.info("The jSSC Native Binary Library loaded successfully.");
        }
        catch(SecurityException e) {
        	logger.error("A security manager is preventing the jSSC Native Binary Library from being loaded.");
        	logger.error(e.toString());
        	return false;
        }
        catch(UnsatisfiedLinkError e) {
        	logger.error("The jSSC Native Binary Library could not be found at the specified filepath.");
        	logger.error(e.toString());
        	return false;
        }
        catch(Exception e) {
        	logger.error("An unknown error occurred while loading the jSSC Native Binary Library.");
        	logger.error(e.toString());
        	return false;
        }
        return true;
