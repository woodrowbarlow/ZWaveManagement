...
public class SerialPortList {
	...
	private static final Logger logger = LoggerFactory.getLogger(SerialPortList.class);
    ...
    public static String[] getPortNames(String searchPath, Pattern pattern, Comparator<String> comparator) {
    	logger.debug("Method getPortNames: Searching on '", searchPath, "' for pattern '", pattern, "'");
    ...
    private static String[] getUnixBasedPortNames(String searchPath, Pattern pattern, Comparator<String> comparator) {
        ...
        if(dir.exists() && dir.isDirectory()){
        	logger.debug("Search path exists!");
        	logger.debug("Listing matched items in search path: ");
                ...
                for(File file : files){
                    ...
                    if(pattern.matcher(fileName).find()){
                        ...
                        if(portHandle < 0 && portHandle != SerialNativeInterface.ERR_PORT_BUSY){
                        	logger.debug("    note: could not open port, did not add to list.");
                            continue;
                        }
